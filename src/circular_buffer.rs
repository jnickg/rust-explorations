...
pub struct MyCicularBuffer<T: BufferElement, const N: usize> {
...
impl<T: BufferElement, const N: usize> Default for MyCicularBuffer<T, N> {
    fn default() -> Self {
        Self {
            data: [T::default(); N],
            size: 0,
...
impl<T: BufferElement, const N: usize> MyCicularBuffer<T, N> {
...
impl<T: BufferElement, const N: usize> Index<usize> for MyCicularBuffer<T, N> {
    type Output = T;
...
impl<T: BufferElement, const N: usize> IndexMut<usize> for MyCicularBuffer<T, N> {
...
mod tests {
    ...
    fn circular_buffer_appends_until_full_when_non_overwriting() {
        let mut buffer = MyCicularBuffer::<u32, 3>::new_empty();
    ...
    fn circular_buffer_appends_forever_when_overwriting() {
        let mut buffer = MyCicularBuffer::<u32, 3>::new_empty_overwriting();
        assert_eq!(buffer.append(1), Ok(1));
        assert_eq!(buffer.append(2), Ok(2));
        assert_eq!(buffer.append(3), Ok(3));
    ...
    fn circular_buffer_indexes_repeatedly() {
        let buffer = MyCicularBuffer::new([1, 2, 3]);
    ...
    fn circular_buffer_mutates_repeatedly() {
        let mut buffer = MyCicularBuffer::new([1, 2, 3]);
        buffer[0] = 4;
        buffer[1] = 5;
        buffer[2] = 6;
